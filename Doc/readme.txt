...
/*  实验八：互斥量降低优先级翻转危害实验 互斥带优先级继承（FreeRTOS.h中的宏定义configUSE_MUTEXES 置1）*/


/*  实验九：事件实验，按键触发事件对应位置1，当两个按键均使目标位置1时，LED_BLUE翻转.				 */
/*  事件的作用类似于信号量，用于任务之间的联系，但是信号量只能一对一，事件可以一对多    				 */
/*  例如：在启动重要机器时，需要将所有需要的东西都初始化好再启动，这时候就要用事件来触发。			 */
...
/* 		  2.创建软件定时器，如果创建成功就激活														 */
/* 		  3.分别创建其对应回调函数																 */
/*  从滴答定时器的数值可以看出，软件定时器的时间并不像硬件那样准确，可能会被其他任务所打断影响。   	 */
